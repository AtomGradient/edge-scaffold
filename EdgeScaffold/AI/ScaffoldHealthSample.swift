...
enum ScaffoldHealthSample {
    static let namespace = "scaffold.health"
    static let schemaName = "scaffold.health.record"
    static let resourceName = "scaffold_health_sample"

    static let recordTypes = ["workout", "measurement", "medication", "appointment", "symptom"]
    static let timeOfDayValues = ["morning", "midday", "afternoon", "evening", "night"]
    static let moods = ["energized", "normal", "tired", "stressed", "sick"]
    ...
    static func registerSchema() {
        Edge.registerSchema(SchemaDef(
            ...
            fields: [
                ...
                FieldDef(name: "activity", type: .entity, required: true),
                FieldDef(name: "duration_minutes", type: .numeric, required: false),
                FieldDef(name: "calories", type: .numeric, required: false),
                FieldDef(name: "value", type: .numeric, required: false),
                FieldDef(name: "unit", type: .text, required: false),
    ...
    static func loadRecords() throws -> [ScaffoldHealthSampleRecord] {
        let urls = [
            Bundle.main.url(
        ...
        guard let url = urls.first else {
            throw ScaffoldHealthSampleError.resourceMissing(resourceName)
        }
        let data = try Data(contentsOf: url)
        return try JSONDecoder().decode([ScaffoldHealthSampleRecord].self, from: data)
    }
    ...
    static func rawFactID(for sampleID: String) -> String {
        "scaffold.health.raw.\(sampleID)"
    }

    static func classifiedFactID(for sampleID: String) -> String {
        "scaffold.health.classified.\(sampleID)"
    }
}

struct ScaffoldHealthSampleRecord: Decodable, Identifiable {
    let id: String
    let date: String
    let weekday: String
    ...
    var summary: String {
        ...
        if let calories {
            parts.append("calories=\(Int(calories))")
        }
        if let value {
            parts.append("value=\(value)\(unit.map { " \($0)" } ?? "")")
        }
        if let location {
    ...
    var rawPayload: [String: Any] {
        var payload: [String: Any] = [
            "text": summary,
            "source": "edge_scaffold_synthetic_health_sample",
            "date": date,
            "weekday": weekday,
...
extension ScaffoldHealthSample {
...
enum ScaffoldHealthSampleError: LocalizedError {
